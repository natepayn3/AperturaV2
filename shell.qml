...
Scope {
    ...
    readonly property bool shutterModeActive: Config.shutterMode
    ...
    function parseConfig(rawJson) {
        ...
        try {
            ...
            if (parsed.currentWallpaper !== undefined) {
                Config.currentWallpaper = parsed.currentWallpaper;
                globalWallpaperPreview.currentWallpaperPath = parsed.currentWallpaper;
            }
            if (parsed.matugenScheme !== undefined) {
    ...
    IpcHandler { target: "shutter"; function toggle(): void { rootShell.toggleShutterMode(); } }
    ...
    Timer { id: previewDebounceTimer; interval: 50; running: false; repeat: false; property int pendingWorkspace: -1; onTriggered: if (pendingWorkspace !== -1 && globalWorkspacePreview.cardRef) globalWorkspacePreview.cardRef.targetWorkspace = pendingWorkspace }
